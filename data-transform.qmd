...
Vous devrez souvent créer de nouvelles variables ou des statistiques pour répondre à vos questions avec vos données, vous devrez peut-être simplement renommer les variables ou réorganiser les observations pour rendre les données un peu plus faciles à manipuler.
...
Autrement, si vous voulez conserver les autres colonnes lors du filtrage pour des lignes uniques, vous pouvez utiliser l'option `.keep_all = TRUE`.
...
5.  Quels vols ont voyagé sur la distance la plus longue ?
    Lesquels ont voyagé sur la distance la plus courte ?
...
Par défaut, `mutate()` ajoute de nouvelles colonnes sur le côté droit de votre jeu de données, il est donc difficile de voir le résultat ici.
...
Vous pouvez également utiliser `.after` pour ajouter la nouvelle colonne après une certaine variable, et dans les deux cas `.before` et `.after` vous pouvez utiliser le nom de la variable au lieu de sa position.
...
Vous pouvez aussi contrôler les variables conservées avec l'argument `.keep`.
...
Le nouveau nom apparaît sur le côté gauche de `=`, et l'ancien nom apparaît sur le côté droit :
...
2.  Réfléchissez à autant de façons possibles de sélectionner `dep_time`, `dep_delay`, `arr_time`, et `arr_delay` à partir de `flights`.
...
Jusqu'à présent, vous avez appris les fonctions qui s'appliquent sur les lignes et les colonnes.
dplyr devient encore plus puissant lorsqu'on ajoute la capacité à travailler avec des groupes.
...
L'opération groupée la plus importante est un résumé, qui, s'il est utilisé pour calculer une seule statistique résumée, réduit le data frame pour avoir une seule ligne par groupe.
...
Les moyennes et les comptes peuvent vous mener très loin en science de données !
...
Nous voulions la mentionner parce que nous pensons qu'elle a un beau potentiel et qu'elle sera probablement très populaire.
...
1.  Quel transporteur a les pires retards moyens ?
    Défi : parviendrez-vous à distinguer les effets liés aux aéroports peu performants de ceux liés aux compagnies aériennes peu performantes ?
    Pourquoi ou pourquoi pas ?
...
6.  Supposons que nous ayons le petit data frame suivant :
    ...
    a.  Notez à quoi, selon vous, ressemblera le résultat final puis vérifiez si vous aviez raison, et décrivez ce que `group_by()` fait.
    ...
    b.  Notez à quoi, selon vous, ressemblera le résultat final puis vérifiez si vous aviez raison, et décrivez ce que `arrange()` fait.
    ...
    c.  Notez à quoi, selon vous, ressemblera le résultat final puis vérifiez si vous aviez raison, et décrivez ce que le pipeline fait.
    ...
    d.  Notez à quoi, selon vous, ressemblera le résultat final puis vérifiez si vous aviez raison, et décrivez ce que le pipeline fait.
    ...
    e.  Notez à quoi, selon vous, ressemblera le résultat final puis vérifiez si vous aviez raison, et décrivez ce que le pipeline fait.
    ...
    f.  Notez à quoi, selon vous, ressembleront les résultats finaux puis vérifiez si vous aviez raison, et décrivez ce que chaque pipeline fait.
...
Chaque fois que vous faites une agrégation, c'est toujours bien d'inclure un compte (`n()`).
...
Nous allons le démontrer avec des données de baseball contenues dans le package **Lahman**.
...
Vous trouverez une bonne explication de ce problème et comment le surmonter sur <http://varianceexplained.org/r/empirical_bayes_baseball/> et <https://www.evanmiller.org/how-not-to-sort-by-average-rating.html>.
